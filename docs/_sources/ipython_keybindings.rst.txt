...
in addition, to utilizing the expansive prompt_toolkit library.

First we'll initialize a global instance of the shell by running::

   from IPython import get_ipython
   _ip = get_ipython()

That will bind our shell instance to `_ip`, and if we're not running in IPython,
`get_ipython()` will return `None`.


Then, we'll import the function
:func:`IPython.terminal.interactiveshell.create_ipython_shortcuts`
and initialize a
``prompt_toolkit.key_binding.key_bindings.KeyBindings`` instance.


Official Prompt Toolkit Documentation
=====================================

Before we dive straight into the source code, let's check out how
``prompt_toolkit`` describes the process of re-binding keys.
...
Then key bindings are discussed in the context of being filtered through
certain conditions.:
...
run into key binding collisions. A ``prompt_toolkit.key_bindings.DynamicKeyBindings``
might be useful. Bring it together with
``prompt_toolkit.key_bindings.merge_key_bindings``:
...
From ``prompt_toolkit.key_bindings.bindings.defaults``::
...
``prompt_toolkit.key_bindings.keybinding.KeyBindings``
...
The help for ``prompt_toolkit.key_binding.key_bindings.DynamicKeyBindings.get_key_bindings``:
...
``/usr/lib/python3.7/site-packages/IPython/`` for example.

Windows users should locate the root of their python installations similarly.
For users of the ``conda`` environment manager, it may be in a location like:

``C:\tools\miniconda3\Lib\site-packages\IPython``.

.. warning:: Users of conda on Windows.

   Note that there's no directory in between lib and site-packages.
...
Up at the top you have all the keybindings that :mod:`IPython` ships with!
